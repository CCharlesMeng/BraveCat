...
- 母版仍注册在 Home 的 1200×1600 房间画布坐标上（分层合成的对位基准
  ...
  "等于画布尺寸"。
- **实测窗洞并集（A/F 为 clean shell 候选实测，B 暂为控制图）：
  x 50–655、y 92–992**；叠加 ±60px 视差行程后需要内容覆盖
  x −10–715、y 32–1052。x=−10 越出画布左缘，说明满画布母版本身
  不够：**运行时 exterior 层须以 ≥1.10 倍渲染**（恰好提供 ±60px
  横向余量），母版按满画布 1200×1600 出图即可。
- **纵向构图注册（本轮合成 QA 的关键发现）**：F 的窗洞高约 900px，
  几乎纵贯半个画布。江湾 v02 母版按当前 2:3 竖向压缩合成时，窗内
  几乎只剩水面，地平线与对岸小屋被窗洞上沿裁掉（证据：
  `../home-theme-prototypes/2026-08-13/production/*/qa--exterior-parallax-travel--candidate-v01.png`）。
  重制母版时**地平线带须落在画布 y ≈ 520–640**，天空向上至少延伸到
  y 32、地面/水面向下至少到 y 1052，使 A 的中窗与 F 的高窗共用同一
  注册仍完整可读。
...
### 第三、四地点：芦苇湖 v02 与灌田 v02（2026-08-13 晚）

- 芦苇湖：`masters/ext-reed-lake--master--noon-clear--candidate-v02.png`，
  自动裁切偏置 140，远岸树线实测 y 580 在带内；芦岛群居中带，
  三窗型行程 QA 通过。
- 灌田：`masters/ext-irrigated-fields--master--noon-clear--candidate-v02.png`，
  裁切偏置触顶 200，检测行 y 756 **超带**——但检测器咬住的是山脚
  树线/农舍带（最强蓝度骤降行），真正的天际（山脊线）目测 ≈520，
  压在带下缘。视觉行程 QA 三窗型均可读（上天空、中山脊与农舍、
  下水田）。判定：**候选保留、带外测量记录在案**；该场景的注册线
  语义与检测器不一致，若后续跨窗验收发现构图偏低，须重生成原始稿
  把山脊抬到 ~560 而不是调检测器。
- 剩余 6 地点：山谷草地、杉溪、花园暖房、大都会高楼、街区屋顶、
  果园坡地。
